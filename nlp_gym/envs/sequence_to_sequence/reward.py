...
class F1RewardFunction(RewardFunction):
    ...
    def __call__(self, observation: BaseObservation, action: str, targets: List[str]) -> float:
        ...
        # remove "EOS" to compute reward
        if "EOS" in current_action_history:
            current_action_history.remove("EOS")

        # step reward as change in the scores
        # as good actions lead to increase in the scores
        previous_score = self.scoring_fn(targets[:len(prev_action_history)], prev_action_history)
        current_score = self.scoring_fn(targets[:len(current_action_history)], current_action_history)
