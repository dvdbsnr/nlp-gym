...
from nlp_gym.envs.sequence_to_sequence.featurizer import DefaultFeaturizerForSequenceToSequence
from nlp_gym.envs.sequence_to_sequence.observation import ObservationFeaturizer, Observation
from nlp_gym.envs.sequence_to_sequence.reward import F1RewardFunction
...
class SequenceToSequenceEnv(BaseEnv):
    """
    ...
    """
    def __init__(self, vocabulary: List[str], max_steps: int = None, reward_function: RewardFunction = None,
        ...
        if return_obs_as_vector:
            observation_featurizer = DefaultFeaturizerForSequenceToSequence(self.action_space) if observation_featurizer is None \
    ...
    def step(self, action: int) -> Tuple[Union[Observation, np.array], int, bool, dict]:
        ...
        # target text
        target_text = copy.deepcopy(self.current_sample.text)

        # compute reward function
        step_reward = self.reward_function(self.current_sample.observation, action_str, target_text)
